...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    UITabBarController *tapVC = [[UITabBarController alloc] init];
    ...
    secondVC.title = @"扫码";
    UINavigationController *nv2 = [[UINavigationController alloc] initWithRootViewController:secondVC];
    
    
    //setting
    UIViewController *settingVC = [[SettingViewController alloc] init];
    settingVC.title = @"设置";
    UINavigationController *setting = [[UINavigationController alloc] initWithRootViewController:settingVC];

    tapVC.viewControllers = @[nv1,nv2,setting];
    
    tapVC.selectedIndex = 1;
    
    
    _window.rootViewController = tapVC;
